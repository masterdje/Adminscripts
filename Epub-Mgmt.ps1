...
Function Batch-Delete-BigFileinEpub($path)
{
	...
	$list = Get-ChildItem -Path $path -Filter "*.epub" -Recurse -File | Where-Object { $_.Length -gt 1MB } | Select-Object FullName, Length
	...
	{ 
		...
		$cuobj += [PSCustomObject] @{
			...
			Maxfile = $maxfile
			Size = $maxsize
			Suspiscious = if (($maxfile.toupper()  -like "*EPUB*") -OR ($maxfile.toupper()  -like "*CONTENT") -OR ($maxfile.toupper()  -like "*PADDING_IMAGE.JPG") -OR ($maxfile.toupper()  -like "*.EXE*") -OR ($maxfile.toupper()  -like "*XLS*") -OR ($maxfile.toupper()  -like "* MB*")  -OR ($maxfile.toupper()  -like "CONTENT (2)") -OR ($maxfile.toupper()  -like "*PDF*")){$true} else {$false}
		}
